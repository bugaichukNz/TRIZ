...
def _run_solve_job(
    ...
    *,
    ...
    try:
        ...
        solve_jobs.complete(job_id, result)
        analysis_progress.complete(job_id)
        logger.info("Solve job %s completed", job_id)
    except TRIZChainError as exc:
        solve_jobs.fail(job_id, str(exc))
        analysis_progress.fail(job_id, str(exc))
        logger.error("Solve job %s TRIZ error: %s", job_id, exc)
    except Exception as exc:
        solve_jobs.fail(job_id, str(exc))
        analysis_progress.fail(job_id, str(exc))
...
@app.post(
    ...
    tags=["triz"],
    summary="Запустить TRIZ-анализ в фоне",
)
def create_solve_job(
    ...
    """Создаёт фоновую задачу анализа и сразу возвращает job_id."""
    ...
    if chat_session_id:
        try:
            problem, interview_brief = chat.prepare_analyze(
                chat_session_id, user["id"], force=body.force
            )
...
@app.get(
    "/solve/jobs/{job_id}",
    response_model=SolveJobStatusResponse,
    tags=["triz"],
    summary="Статус фонового TRIZ-анализа",
)
def get_solve_job(
    job_id: str,
    user: CurrentUser,
) -> SolveJobStatusResponse:
    job = solve_jobs.get_for_user(job_id, user["id"])
    if job is None:
        raise HTTPException(status_code=404, detail="Задача не найдена.")

    progress_state = analysis_progress.get(job_id)
    if progress_state is not None and job.status == "running":
        pct = progress_state.progress
        stage = progress_state.stage
    else:
        pct = job.progress_pct
        stage = job.stage

    result = SolveResponse(**job.result) if job.result is not None else None
...
def create_session_entry(
    ...
    row = store.add_entry(body.problem, body.result, time_display=body.time, user_id=user["id"])
    return HistoryEntry(**row)
